...
/* Optimized for Professional Layout, Legal Safety & Hitech UI + ADS INTEGRATION */

// --- CONFIGURATION FOR ADS ---
const INFO_ADS_CONFIG = {
    SOCIAL_BAR_URL: "//pl28285623.effectivegatecpm.com/8f/bd/f6/8fbdf667a2a2e1609a5d4f38e0105d34.js",
    SOCIAL_LIMIT_PER_DAY: 6, // Max 6 times per day on info pages
    NATIVE_SCRIPT_URL: "//pl28285595.effectivegatecpm.com/85c8e4eb0a60d8ad0292343f4d54b04b/invoke.js",
...
// B. BIG AD BOXES (HOME STYLE REPLICA - NATIVE ADS)
// Top Ad Box
const adSpaceTop = `
<div class="native-ad-module" style="background: linear-gradient(180deg, rgba(15,23,42,1) 0%, rgba(15,23,42,0.8) 100%); border: 1px solid rgba(255, 215, 0, 0.2); border-radius: 16px; margin-bottom: 25px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.5); position: relative;">
    ...
    <div id="ad-frame-top" style="min-height: 280px; width: 100%; display: flex; align-items: center; justify-content: center; padding-top: 30px;"></div>
</div>`;
...
<div class="native-ad-module" style="background: linear-gradient(180deg, rgba(15,23,42,1) 0%, rgba(15,23,42,0.8) 100%); border: 1px solid rgba(255, 215, 0, 0.2); border-radius: 16px; margin-top: 35px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.5); position: relative;">
    ...
    <div id="ad-frame-bottom" style="min-height: 280px; width: 100%; display: flex; align-items: center; justify-content: center; padding-top: 30px;"></div>
</div>`;
...
function openInfoPage(pageKey) {
    ...
    if(page && contentEl) {
        ...
        // 3. ⚡ INJECT ADS & TRIGGER SOCIAL BAR ⚡
        setTimeout(() => {
...
// 🔥 POWERFUL IFRAME INJECTOR (FULL SIZE BOX) 🔥
    ...
    doc.write(`
        ...
        <head>
            <style>
                ...
                /* Scale Adsterra to look bigger */
                #container-${INFO_ADS_CONFIG.NATIVE_KEY} { transform: scale(1.1); }
            </style>
        </head>
        <body>
            <div id="container-${INFO_ADS_CONFIG.NATIVE_KEY}"></div>
            <script async="async" data-cfasync="false" src="${INFO_ADS_CONFIG.NATIVE_SCRIPT_URL}"></script>
        </body>
        </html>
    `);
    doc.close();
...
// 🔥 SOCIAL BAR LOGIC (LIMITED PER DAY) 🔥
function triggerSocialBar() {
    // 1. Check Date
    const today = new Date().toDateString();
    const lastDate = localStorage.getItem('info_ad_date');
    
    if (lastDate !== today) {
        // Reset Counter for new day
        localStorage.setItem('info_ad_date', today);
        localStorage.setItem('info_ad_count', 0);
    }

    // 2. Check Count
    let count = parseInt(localStorage.getItem('info_ad_count') || "0");
    
    if (count < INFO_ADS_CONFIG.SOCIAL_LIMIT_PER_DAY) {
        // Inject Social Bar
        console.log("🚀 Ads: Showing Social Bar on Info Page...");
        const s = document.createElement('script');
        s.src = INFO_ADS_CONFIG.SOCIAL_BAR_URL;
        s.async = true;
        document.body.appendChild(s);
        
        // Update Count
        localStorage.setItem('info_ad_count', count + 1);
    }
}
